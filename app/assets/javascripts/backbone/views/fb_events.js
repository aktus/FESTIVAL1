App.FbEvents = App.BaseView.extend({
  ...
  getEvents: function () {
    ...
    query = "SELECT eid, name, start_time, location FROM event WHERE eid IN (SELECT eid FROM event_member WHERE uid=392083150815844) AND start_time > " + now + " ORDER BY start_time ASC LIMIT 10";
